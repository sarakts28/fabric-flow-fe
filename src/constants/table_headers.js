export const ROUTES_TABLE_HEADERS = [
  { label: "Route Name", index: "routeName" },
  { label: "Route Type", index: "routeType" },
  { label: "Cost Per Meter", index: "costPerMeter" },
  { label: "Lead Time (Days)", index: "leadTime" },
  { label: "Action", index: "action" },
];

export const CATEGORIES_TABLE_HEADERS = [
  { label: "Category Name", index: "categoryName" },
  { label: "Category Season", index: "season" },
  { label: "Action", index: "action" },
];

export const ARTICLES_PLANNING_TABLE_HEADERS = [
  { label: "Planning Name", index: "planningName" },
  { label: "Article Id", index: "articleId" },
  { label: "Planning Route Id", index: "PlanningRouteId" },
  { label: "Status", index: "status" },
  { label: "Late", index: "late" },
  { label: "Order Slip", index: "orderSlip" },
  { label: "Process Days", index: "processDays" },
  { label: "Total Payment", index: "totalPayment" },
  { label: "When Process Start", index: "whenProcessStart" },
  { label: "When Process End", index: "whenProcessEnd" },
  { label: "Action", index: "action" },
];

export const ARTICLES_TABLE_HEADERS = [
  { label: "Article No.", index: "articleNo" },
  { label: "Article Name", index: "articleName" },
  { label: "Article Description", index: "articleDescription" },
  { label: "Images", index: "images" },
  { label: "Active Status", index: "activeStatus" },
  { label: "Fabric Type", index: "fabricType" },
  { label: "Measurement Type", index: "measurementType" },
  { label: "Total Quantity", index: "totalQuantity" },
  { label: "Designer Name", index: "designerName" },
  { label: "Stores Assigned", index: "storesAssigned" },
  { label: "Quantity Dispatched", index: "quantityDispatched" },
  { label: "Price", index: "price" },
  { label: "status", index: "status" },
  { label: "Action", index: "action" },
];
